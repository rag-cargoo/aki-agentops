# Repository Architecture Gap Map (2602)
...
## Scope
- 본 문서는 `2602` 루트 저장소 아키텍처 진단 결과를 4축으로 정리한다.
...
| --- | --- | --- | --- | --- |
| 경계(Repository vs Project) | `2602`가 운영 허브 + 일부 프로젝트 경로를 함께 다룸 | 제품 코드와 거버넌스 변경이 같은 리뷰/훅 컨텍스트에 섞이기 쉬움 | 제품 코드는 독립 레포, `2602`는 sidecar 문서/운영 스크립트 전용 | 경계 전환 중 잘못된 커맨드 대상(`git/gh`)으로 오조작 가능 |
