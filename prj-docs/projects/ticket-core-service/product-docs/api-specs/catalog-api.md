...
공연 카탈로그의 소속사(Entertainment), 아티스트(Artist), 주최사(Promoter), 공연장(Venue) 도메인 CRUD API를 정의합니다.
...
### 1.1. 엔터테인먼트(소속사) 생성
- **Endpoint**: `POST /api/entertainments`
- **Description**: 엔터테인먼트(소속사)를 생성합니다.

**Request Body**

| Field | Type | Required | Description |
| :--- | :--- | :--- | :--- |
| `name` | String | Yes | 엔터테인먼트 이름 (중복 불가) |
...
### 1.2. 엔터테인먼트 목록 조회
- **Endpoint**: `GET /api/entertainments`
- **Description**: 전체 엔터테인먼트 목록을 조회합니다.

### 1.2-1. 엔터테인먼트 검색/필터/정렬 + 페이징 조회
- **Endpoint**: `GET /api/entertainments/search`
- **Description**: 엔터테인먼트 이름/국가코드 기준 검색, 정렬, 페이징을 수행합니다.

**Parameters**

| Location | Field | Type | Required | Description |
| :--- | :--- | :--- | :--- | :--- |
| Query | `keyword` | String | No | 엔터테인먼트 이름/국가코드 검색 키워드 |
...
### 1.3. 엔터테인먼트 단건 조회
- **Endpoint**: `GET /api/entertainments/{id}`
- **Description**: 엔터테인먼트 단건 정보를 조회합니다.

### 1.4. 엔터테인먼트 수정
- **Endpoint**: `PUT /api/entertainments/{id}`
- **Description**: 엔터테인먼트 이름/메타데이터를 수정합니다.

**Request Body**

| Field | Type | Required | Description |
| :--- | :--- | :--- | :--- |
| `name` | String | Yes | 엔터테인먼트 이름 |
| `countryCode` | String | No | 국가 코드 |
| `homepageUrl` | String | No | 홈페이지 URL |

### 1.5. 엔터테인먼트 삭제
- **Endpoint**: `DELETE /api/entertainments/{id}`
- **Description**: 엔터테인먼트를 삭제합니다.
...
| `entertainmentId` | Long | Yes | 소속 엔터테인먼트 ID |
...
| Location | Field | Type | Required | Description |
| :--- | :--- | :--- | :--- | :--- |
| Query | `keyword` | String | No | 아티스트/소속 엔터테인먼트 검색 키워드 |
| Query | `entertainmentId` | Long | No | 소속 엔터테인먼트 ID 필터 |
| Query | `page` | Integer | No | 페이지 번호(기본값 `0`) |
| Query | `size` | Integer | No | 페이지 크기(기본값 `20`) |
| Query | `sort` | String | No | 정렬 규칙(`id|name|displayName|genre|debutDate|entertainmentName`,`asc|desc`) 예: `name,asc` |
...
- **Description**: 아티스트 이름/프로필/소속 엔터테인먼트를 수정합니다.
