...
- Status: DONE
- 결정사항:
  - 아티스트 소속 도메인 명칭은 `Entertainment`로 정규화했다.
...
- Status: DONE
- 핵심 발견:
  - `Artist -> Entertainment` 소속 구조는 이미 존재한다.
...
- 적용 계획:
  - 1차: `Entertainment/Artist` Admin 목록용 `find/search/paging/sort`를 QueryDSL custom repository로 구현한다.
  - 2차: `Concert Admin` 목록/필터(아티스트/엔터테인먼트/상태/날짜 범위) 조회에 QueryDSL을 확장한다.
...
- Phase B (다음):
  ...
  - `Entertainment/Artist` QueryDSL 검색/페이지네이션 도입(1차 완료)
...
- Status: DONE
- 구현:
  - `Entertainment/Artist` QueryDSL custom repository를 추가했다.
  - Admin 조회 API를 확장했다:
    - `GET /api/entertainments/search`
    - `GET /api/artists/search`
  - 페이징 응답 DTO를 추가했다:
    - `EntertainmentSearchPageResponse`
    - `ArtistSearchPageResponse`
- 검증:
  - `./gradlew compileJava` PASS
  - `./gradlew test --tests '*Entertainment*' --tests '*Artist*'`는 기존 Redis 통합테스트 환경 의존으로 실패(`RedisConnectionException`)
