...
- [x] TWA-SC-014 Admin CRUD 계약 정렬(필드 완전노출/업로드 플로우/옵션 계약 동기화)
- [ ] TWA-SC-015 인증 만료 자동복구(401 refresh 재시도) + 세션 상태 일치 보장

## Current Items
- TWA-SC-015 인증 만료 자동복구(401 refresh 재시도) + 세션 상태 일치 보장
  - Status: TODO
  - Description:
    - access token 만료(`AUTH_TOKEN_EXPIRED`)를 정상 흐름으로 처리하고, refresh token으로 무중단 복구한다.
    - 서비스/체크아웃/계정 API 및 좌석 실시간 구독 등록 API에서 공통 재시도 정책을 적용한다.
    - \"로그인 UI 상태\"와 \"실제 인증 가능 상태\"가 어긋나지 않도록 세션 동기화 규칙을 고정한다.
  - TODO:
    - [ ] 401 + `AUTH_TOKEN_EXPIRED` 감지 시 refresh + 원요청 1회 재시도 공통 계층 도입
    - [ ] refresh single-flight(동시 1회) + 대기열 처리
    - [ ] refresh 실패 시 세션 clear + 로그인 유도 UI 전환
    - [ ] WS 구독 등록/해제 API refresh 재시도 경로 적용
    - [ ] 만료 재현 QA(30분 방치 + refresh 만료) 실행/기록
  - 완료 체크 규칙:
    - 모든 TODO를 `[x]`로 바꾼 뒤에만 Status를 `DONE`으로 변경한다.
    - 완료 시 Evidence(코드 경로 + `lint/typecheck/build` + 재현/복구 로그)를 반드시 남긴다.
  - Evidence:
    - issue: `https://github.com/rag-cargoo/ticket-web-app/issues/15`
    - meeting note: `prj-docs/projects/ticket-web-app/meeting-notes/2026-03-03-auth-token-expiry-recovery-and-session-sync-plan.md`
...
- TWA-SC-013 서비스 카드 실시간 상태(서버시간 기준) WS 채널 + fallback 계약 도입
  ...
  - Progress (2026-03-01 frontend hotfix update):
    - 오픈 임박 YouTube iframe의 기본 컨트롤(재생/음소거/전체화면) 비노출 원인을 제거했다(`controls=1`, `fs=1`, pointer-events/mask 제거).
    - 스크롤 위치 변화만으로 재음소거되지 않도록 mute 조건에서 `isOpeningMediaInView` 의존을 제거했다.
    - player `Ended(0)` 감지 시 `seekTo(0)+playVideo` 재요청과 재생 keep-alive를 추가해 종료 오버레이 노출 가능성을 낮췄다.
    - 로컬 수동 검증에서 사용자 확인 코멘트("이제 제대로 되네")를 수신해 현상 복구를 확인했다.
  - Progress (2026-03-02 backend scheduler fix):
    - Demo Rebalancer 자동 실행 누락 원인을 확인했다(`defaultIntervalMinutes`는 UI/상태값이고 서버 스케줄과 미연결).
    - 백엔드에 `매시 00분` cron 실행과 `서버 시작 후 60초 이내 1회(기본 30초)` startup trigger를 추가했다.
    - 운영 배포 후 `/api/dev/demo/rebalancer/status`에서 startup 자동 실행 이력을 확인했다.
  ...
  - Evidence:
    ...
    - `prj-docs/projects/ticket-web-app/meeting-notes/2026-03-01-service-urgency-youtube-controls-and-audio-unlock-fix.md`
    - `prj-docs/projects/ticket-web-app/meeting-notes/2026-03-02-demo-rebalancer-hourly-cron-startup-trigger-fix.md`
...
- TWA-SC-014 Admin CRUD 계약 정렬(필드 완전노출/업로드 플로우/옵션 계약 동기화)
  - Status: DONE
  - Description:
    - Admin 콘서트/옵션 CRUD에서 누락된 표시 필드를 백엔드 계약 기준으로 확장한다.
    - 프론트 요청 payload/응답 파서를 백엔드 DTO와 정렬해 저장값/표시값 불일치를 줄인다.
    - thumbnail 업로드를 등록/수정 운영 흐름에서 일관되게 사용할 수 있도록 UX를 정리한다.
  - Progress (2026-03-02):
    - `AdminPage` 콘서트/옵션 모달을 계약 필드 기준으로 재구성하고, artist/promoter/venue를 카탈로그 검색 + 선택 UX로 전환했다.
    - 콘서트 등록/수정 모달에서 썸네일 파일 선택을 받아 저장 직후 `multipart/form-data` 업로드를 연계했다.
    - Option CRUD에서 `maxSeatsPerOrder` 직접 입력을 제거하고 `venueId` 중심으로 정렬했으며, 상한은 Sales Policy(`maxReservationsPerUser`) 기준으로 동기화했다.
    - `admin-concert-client` 요청/응답 계약을 백엔드 DTO 기준으로 정렬했다(옵션 body의 `maxSeatsPerOrder` 제거).
    - 서비스 checkout에서도 판매정책 값을 회차 상한으로 반영하도록 보정했고, Admin 폼에서 고정 정책(`artistGenre=K-POP`)과 자동 반영(`promoter.countryCode -> entertainmentCountryCode`)을 적용했다.
  - Scope:
    - Concert CRUD: artist/promoter/runtime/thumbnail 관련 필드 노출 정리
    - Option CRUD: concertDate/ticketPrice/venue 필드 노출 및 입력 정렬
    - API client: 타입/파서/request body 계약 정렬
  - Evidence:
    - `https://github.com/rag-cargoo/ticket-web-app/issues/13` (closed)
    - `https://github.com/rag-cargoo/ticket-web-app/pull/14` (merged)
    - merge commit: `4debe2c7f32e9cd56cad797594c30d835282fa5b`
    - `prj-docs/projects/ticket-web-app/meeting-notes/2026-03-02-admin-crud-contract-kickoff.md`
    - `workspace/apps/frontend/ticket-web-app/src/pages/AdminPage.tsx`
    - `workspace/apps/frontend/ticket-web-app/src/pages/service/ServiceCheckoutModal.tsx`
    - `workspace/apps/frontend/ticket-web-app/src/shared/api/admin-concert-client.ts`
    - `workspace/apps/frontend/ticket-web-app/src/shared/api/admin-catalog-client.ts`
    - `workspace/apps/frontend/ticket-web-app/src/styles.css`
    - `branch(root): feat/twa-admin-crud-hardening-20260301`
    - `branch(ticket-web-app): feat/admin-crud-contract-hardening-20260301`
    - `npm run typecheck` (pass)
    - `npm run lint` (pass)
    - `npm run build` (pass)
