...
```jsonl
{"time":"2026-05-15T10:00:00+09:00","task":"TASK-001","event":"backend_started","agent":"backend"}
...
3. 필요한 skills를 계산한다.
4. 해당 skills와 프로젝트 worktree를 컨테이너에 마운트한다.
5. 역할별 Codex prompt를 생성한다.
6. Docker 컨테이너를 실행한다.
7. 성공/실패 이벤트를 기록한다.
8. 실패 시 재시도하거나 needs_human 상태로 멈춘다.
...
/opt/aki-agentops/skills      skills
...
3. backend-agent 실행
4. backend_done 이벤트 기록
5. orchestrator가 test-agent 자동 실행
6. test-agent가 테스트 실행 후 test_passed/test_failed 기록
7. reviewer-agent가 diff 검토
```
...
### Phase 3: skill resolver 구현

- skill-index.json 작성
- role/file_pattern/keyword 기반 skills 선택
- 전체 skills 마운트 방식에서 시작
- 필요한 skills만 임시 디렉터리에 복사하는 방식으로 확장

### Phase 4: Git worktree 연동
