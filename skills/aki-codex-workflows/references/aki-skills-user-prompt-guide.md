...
- 프론트 e2e 목록/선택 실행: `프론트 Playwright 테스트 목록 보여주고 scope별로 실행해줘`
...
### 13) 프론트 복귀(장기 공백) 빠른 재개

- 프롬프트:
  - `프론트 작업 복귀용 체크리스트부터 보여줘`
  - `프론트 테스트 목록 먼저 보여주고 내가 scope 고르면 실행해줘`
- 기대 결과:
  - Active frontend 프로젝트의 feature spec/runbook 경로를 제시한다.
  - Playwright scope 목록(`smoke/nav/contract/all`)을 먼저 보여준다.
  - 선택된 scope만 실행하고 로그/요약 경로를 보고한다.
