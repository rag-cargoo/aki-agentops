...
## 오케스트레이션 경계
- 이 스킬은 pre-commit 도메인 실행(모드 전환/체인 검증)만 담당한다.
- 커밋 전후 다른 스킬과의 호출 순서/분기/종료판정은 `aki-codex-workflows` 문서를 권위 소스로 따른다.
...
1. `quick`:
   ...
   - 경량 검증/경고 중심
2. `strict`:
   - 중요 커밋(정책/런타임/워크플로우/훅), PR 머지 직전, 릴리즈 직전, 대규모 리팩토링 완료 시 사용
...
- `.codex/tmp/` 밖의 temp-like 산출물(`*.log`, `*.tmp`, 대시보드 HTML/PNG)이 stage되면 warning 출력(차단 아님)
