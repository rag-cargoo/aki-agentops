...
### 운영 원칙
1.  **가시성 보장 (Visibility)**: 새로 생성된 모든 기술 문서(`.md`)는 반드시 `github-pages/sidebar-manifest.md`에 등록되어야 한다. 등록되지 않은 문서는 "존재하지 않는 문서"로 간주한다.
...
### 사이드바(github-pages/sidebar-manifest.md) 작성 규격
*   **프로젝트 단위 그룹화**: 각 마이크로서비스나 독립 모듈별로 섹션을 분리한다.
*   **중요도 순 배치**: README, Project Agent, Task 등 현재 진행 상황을 알 수 있는 문서를 상단에 배치한다.
