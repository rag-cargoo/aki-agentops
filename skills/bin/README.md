...
직접 실행도 `skills/bin/codex_skills_reload/` 경로를 기준으로 한다.
...
2. `sync-skill.sh`
3. `codex_skills_reload/` (리로드/프로젝트 전환 구현)

## Safety
1. 세션 시작 시 `./skills/bin/codex_skills_reload/session_start.sh`가 `skills/bin` 무결성을 점검한다.
2. 스크립트를 수정/추가했다면 `chmod +x skills/bin/*.sh skills/bin/codex_skills_reload/*.sh` 후 재실행한다.
