...
3. 중복이 없으면 이슈를 생성하고, 중복이 있으면 기존 이슈를 갱신한다.
4. 라벨 정책을 적용한다(없으면 생성, 있으면 재사용).
5. 프로젝트를 사용할 경우 생성/갱신 이슈를 프로젝트 상태와 동기화한다.
6. PR 링크가 있으면 이슈-PR 상호 참조를 정리한다.
7. 회의록 원문에 Issue/PR 링크를 역기록한다.
